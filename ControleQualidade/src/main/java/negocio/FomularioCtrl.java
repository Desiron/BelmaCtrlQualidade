...
public class FomularioCtrl implements Serializable{
	...
	public String visuRota(Formulario form) {
		...
		VisuFormTemp temp1 = new VisuFormTemp();
		VisuFormTemp temp2 = new VisuFormTemp();
		temp1.setLabel("Hora Umidade: ");
		temp2.setLabel("ID: ");
		temp2.setValores(new ArrayList<String>());
		for(int i=0; i<form.getRotas().size(); i++) {
			System.out.println("Hora: "+form.getRotas().get(i).getHoraUmidade());
			System.out.println("Hora: "+form.getRotas().get(i).getId());
			temp2.getValores().add(Integer.toString(form.getRotas().get(i).getId()));
		}
		visuTemp.add(temp1);
		visuTemp.add(temp2);
